...
function migrate(): AppState {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed?.v === 2) return { ...defaultState(), ...parsed };
    }
...
export default function Home() {
  ...
  useEffect(() => { if (ready) history.replaceState(null, '', `#${tab}`); }, [tab, ready]);
  ...
  const go = (next: Tab) => { setTab(next); setMoreOpen(false); };
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
          ...
          <section className="grid gap-4 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
            <div>
              ...
              <p className="mt-2 text-sm text-[#A6ADBD]">No login. No backend. Versioned localStorage v2 with export/import backup.</p>
            </div>
